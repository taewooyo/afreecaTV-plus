...
export default function App(props: { nicks: User[]; ids: User[]; toggle: ToggleData; collector: ChatCollectorData }) {
    ...
    const addNickBtnClick = () => {
        ...
        const newNicks = [...nicknames, nickInput.current.value]
            .map(nickname => ({isNickname: true, user: nickname}));
    ...
    const addIdBtnClick = () => {
        ...
        // if (nicks.includes(nickInput.current.value)) return;
        const newIds = [...userIds, idInput.current.value]
            .map(id => ({isNickname: false, user: id}));
    ...
    const nicknameClick = (e: React.MouseEvent<HTMLDivElement>) => {
        const newNicks = nicks.filter((item) => item.user !== e.currentTarget.innerHTML);
        chrome.storage.local.set({nicks: newNicks}, () => {
            setNicks(newNicks);
        });
    };
    ...
    const idClick = (e: React.MouseEvent<HTMLDivElement>) => {
        const newIds = ids.filter((item) => item.user !== e.currentTarget.innerHTML);
        chrome.storage.local.set({ids: newIds}, () => {
            setIds(newIds);
        });
    };
    ...
    return (
        <div className="wrapper" style={
            ...
            <div>
                <ul>
                    <Toggle onChange={() => changeCollector()} label="채팅 콜렉터" value={collector.isUse}/>
                    <Toggle onChange={() => changeToggle("streamer")} label="BJ" value={toggle.streamer}/>
                    <Toggle onChange={() => changeToggle("manager")} label="매니저" value={toggle.manager}/>
                    <Toggle onChange={() => changeToggle("topfan")} label="열혈팬" value={toggle.topfan}/>
                    <Toggle onChange={() => changeToggle("gudok")} label="구독팬" value={toggle.gudok}/>
                    <Toggle onChange={() => changeToggle("fan")} label="팬클럽" value={toggle.fan}/>
                    <Toggle onChange={() => changeToggle("user")} label="일반유저" value={toggle.user}/>
                </ul>
                ...
                <br/>
                <button onClick={addNickBtnClick} id="add-btn" style={
                    {
                        border: "0",
                        marginLeft: "5px",
                        marginBottom: "5px",
                ...
                <input ref={idInput} id="id-input" type="text" placeholder="아이디를 입력하세요"
                ...
                <br/>
                <button onClick={addIdBtnClick} id="add-btn" style={
                    {
                        border: "0",
                        marginLeft: "5px",
                        marginBottom: "5px",
