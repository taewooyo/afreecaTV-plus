      ...
      (mutations: MutationRecord[]) => {
        mutations.forEach((mutation: MutationRecord) => {
          mutation.addedNodes.forEach((n) => {
            ...
            const thumbnailBottomPosition =
              streamerStateBoxLive.style.cssText.split("; ")[0];
            const thumbnailPosition = Number(
              thumbnailBottomPosition.slice(
                5,
                thumbnailBottomPosition.length - 3
              )
            );
              ...
              if (elem.tagName == "A") {
                ...
                streamerNameList.forEach((value, _) => {
                  if (url.indexOf(value) != -1) {
                    ...
                    const thumnailQuery = liveElement[0]
                      .slice(baseIndex)
                      .split(", ")[1]
                      .slice(0);
                    ...
                    thumbnail.style.top =
                      String(thumbnailPosition + height) + "px";
                    thumbnail.style.zIndex = "999";

                    const img = document.createElement("img");
                    img.src =
                      "https://liveimg.afreecatv.com/m/" + thumnailQuery;
